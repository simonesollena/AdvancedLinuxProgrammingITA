...
Se ti piace Emacs e vuoi imparare qulcosa in più a riguardo delle sue funzionalità avanzate, dovresti
...
già del testo, Emacs indenta il testo. Così, per esempio, supponi di aver scritto la seguente:
\begin{listcodeC}
int main ()
{
printf ("Hello, world\n");
...
\subsection{Evidenziatura della sintassi}\label{subsec:1.1.3} % 1.1.3
...
supponga che vengano compilati e quindi linkati assieme per produrre un programma chiamato
...
directory per i files sorgente chiamata src ed un'altra chiamata \texttt{include}. Dovresti compilare
...
L'idea di base che sta dietro a \texttt{make} è semplice. Dici a \texttt{make} quali \textit{obiettivi} vuoi compilare,
...
\texttt{reciprocal.o} è stata cambiata.
...
comportando nel modo in cui pensi dovrebbe comportarsi. Lo farai molto.\footnote{...finché il tuo
...
quali linee nel codice sorgente, come stampare variabili locali, e così via
...
stessa a quello che è descritto nella pagina di manuale.
