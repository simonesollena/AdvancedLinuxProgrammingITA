...
I thread, come i processi, sono un meccanismo per permettere ad un programma di
fare più di una cosa allo stesso tempo. Come con i processi, i thread appaiono
...
nuovo processo ed in quel processo crea un singnolo thread, che esegue il
...
riferimento agli ID di thread nei programmi C o C++, si usa il tipo
\texttt{pthread\_t}.
...
la funzione ritorna, il thread esiste. Su GNU/Linux, le funzioni thread prendono
un singolo parametro, di tipo \texttt{void*}, ed hanno un tipo di ritorno
\texttt{void*}. Il parametro è l'\textit{argormento del thread}: GNU/Linux passa
...
Il programma nel listato 4.1 crea un thread che stampa x continuamente sullo
standard error. Dopo aver chiamato \texttt{pthread\_create}, il thread
principale stampa o continuamente sullo standard error.\\
...
\end{listcodeBash}
Prova ad eseguirlo per vedere cosa accade. Nota il modello di x ed o
imprevedibile, come Linux schedula alternativamente i due thread.
...
L'argomento dei thread fornisce un metodo conveniente per passare i dati ai
thread. Poiché il tipo dell'argomento è \texttt{void*}, comunque, non puoi
passare moti dati direttamente tramite l'agomento. Piuttosto, usa l'argomento
...
vengono creati due nuovo thread, uno per stampare x e l'altro per stampare o.
...
altri due thread non siano complatati. Ciò di cui abbiamo bisogno è una funzione
simile a \texttt{wait} che aspetti che finisca un thread piuttosto che un
processo. La funzione è \texttt{pthread\_join}, che prende due argomente: l'ID
del thread da aspettare ed un puntatore ad una variabile \texttt{void*} che
riceverà il valore di ritorno del thread finito. Se non ti preoccupi del valore
...
Morale della storia: Assicurati che ogni dato che passi a un thread per
riferimento non sia deallocato, \textit{anche da un diverso thread}, fino a che
non sei sicuro che il thread sia compelto con esso. Ciò è vero sia per variabili
...
valore di ritorno del thread verrà mesto nella locazione puntata da
...
passi un puntatore nullo, gli attributi di default dei thread sono usati per
configurare il nuovo thread. Comunque, puoi creare e personalizzare l'oggetto
...
\begin{enumerate}
\item{Creare un oggetto \texttt{pthread\_attr\_t}. La maniera più semplice è
dichiarare una variabile automatica di questo tipo.}
...
Per molti compiti delle applicazioni programmate in GNU/Linux, solo un attributo
di thread è tipicamente di interesse (gli altri attributi disponibili servono
	principalmente per la programmazione realtime specializzata). Questo
attributo è lo \textit{stato distaccato} del thread. Un thread può essere creato
come un \textit{thread congiungibile} (di default) o come un \textit{thread
distaccato}. Un thread congiungibile, come un processo, non è cancellato
...
ritornando dalla sua funzione thread sia chiamado \texttt{pthread\_exit}.
Comunque, è possibile per un thread richiedere ad un altro thread di terminare.
Ciò è chiamato \textit{cancellazione} (\textit{canceling}) di un thread.

Per cancellare un thread, chiama \texttt{pthread\_cancel} pasandogli l'ID del
...
completa o nulla, senza vie di mezzo. per esempio, il thread può allocare,
...
tread controllare se e quando esso può essere cancellato.
...
normale operazione, la migliore strategia è quella di indicare al thread che
...
(che sono descritti nel \autoref{cap:5}\marginpar{capitolo 5}).
...
quando ogni thread esiste, passando il valore specifico di thread corrispondente
...
thread non è cancellata. Se non hai bisogno di una funzione di pulizia, puoi
...
Il listato 4.7 mostra come dovresti implementare ciò. La funzione \texttt{main}
...
Poichè questa è una variabile globale, è condivisa da tutti i thread. Quando
ogni thread comincia ad eseguire la sua funzione thread, esso apre un filedi log
e memorizza il puntatore a file in quella chiave. Successivamente, ognuno di
questi thread può chiamare \texttt{wtrite\_to\_thread\_log} per scrivere un
...
un thread esiste o è cancellato. A volte, benché, può risultare utile
...
di pulizia per assicurarti che a buffer allocato dinamicamente viene ripulito se
il thread termina.\\
...
fuori del loro campo di operabilità, o perchè un blocco è eseguito per
completamento o perché viene lanciata un'eccezione, il C++ si assicura che i
distruttori siano chiamati per queel variabili automatiche che loro hanno. Ciò
...
i distruttori sono chiamati per tutte le variabili automatiche nello stack del
...
thread può girare per un tempo molto lungo, o il sistema può commutare tre i
thread molto velocemente. In un sistema con più processore, il sistema può anche
...
facilmente riprodurre il comportamente che ha causato il problema. Puoi eseguire
...
L'ultima causa di molti bug che riguardano i thread è che i thread cercano di
accedere agli stessi dati. Come detto precedentemente, questo è uno dei più
potenti aspetti dei trhread, ma può anche essere pericoloso. Se un thread è solo
a metà strada dell'aggiornamento di una struttura dati mentre un altro thread
accede alla stessa struttura dati, è facile che ne risulti il caos. Spesso,
...
processare di molti thread concorrenti. La coda di lavori è rappresentata da una
lista collegata di oggetti \texttt{struct job}.

Dopo che ogni trhead termina un'operazione, esso controlla la coda per vedere se
...
operazione \textit{atomiche} (\textit{atomic}). Un'operazione atomica è
indivisibile e non interrompibile; una volta che l'operazione si avvia non può
essere messa in pausa o interrotta finché non completa, e nessun altra 
...
Implementare ci richiede il supporto da parte del sistema operativo. GNU/Linux
...
\textit{sbloccato} \--- permesso di riprendere l'esecuzione. GNU/Linux
...
mutex dovrebbe essere inizializzata solo una volta. Questo frammento di codice
...
\end{listcodeC}
Un thread può cercare di bloccare un mutex chiamando in esso
...
solo uno dei thread fermati (scelto in modi imprevedibile) viene sbloccato e gli
...
quando l'intere sequenza di verifica della coda e rimozione del lavoro è
