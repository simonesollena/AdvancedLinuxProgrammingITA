...
chiamati \textit{deadlocks}. Un deadlock, stallo, si verifica quando uno o più thread sono
...
un mutex sarà sospeso finché il mutex non sarà sbloccato. Ma poiché il thread che
...
esso dal thread che tiene la chiusura; quel thread deve effettuare lo stesso numero di
chiamate a \texttt{pthread\_mutex\_unlock} affinché che il mutex venga attualmente
sbloccato ed un altro thread abbia il permesso di bloccarlo.
...
degli altri due tipi, bisogna prima creare un oggetto attributo mutex dichiarando una
...
Passare un puntatore a questo oggetto attributo a
\texttt{pthread\_mutex\_destroy} per creare un mutex di questo tipo, e quindi
distruggi l'oggetto attributo con \texttt{pthread\_mutexattr\_destroy}

Questa sequenza di codice illustra la creazione di un mutex con controllo d'errore,
per esempio:
\begin{listcodeC}
