...
\firstsentence{Q}{uesto capitolo copre alcune tecniche di base che usano molti programmatori}
GNU/Linux.
...
cui si comporta il programma, mentre gli altri argomenti forniscono gli input (per esempio, i nomi
dei file di input).
...
descrittore di file 1 (\texttt{stdout}). Nota che \texttt{2>\&1} deve seguire una ridirezione di file (il primo
...
di tipo \texttt{char**}, è un array di puntatori a stringhe di caratteri che terminano con NULL. Ogni stringa
...
che non ci saranno più dei descrittori di file per quel file. Per questi motivi il tuo programma
...
non usare queste funzioni poiché
...
scrivere programmi che si comportino decentemente in situazioni errori è ancora più difficile.
...
penalità di prestazioni e decidere successivamente di compilare con \texttt{NDEBUG} definito. Ciò
...
\end{verbatim}
Fornisce molte più informazioni rispetto al messaggio di errore che ne risulterebbe se il tuo
...
è andato storto. Poiché tutte le chiamate di sistema usano la stessa variabile \texttt{errno} per
...
appropriata in caso di fallimento può essere quella di stampare un messaggio di errore, di cancellare
...
file dato da \texttt{path} all'utente da \texttt{user\_id}. Se la chiamata fallisce, il programma esegue l'azione
...
corrente ma non terminare il programma poiché potrebbe essere possibile un ripristino
...
Se decidi di ritornare mentre l'esecuzione è nel mezzo della funzione, è importante assicurarsi che ogni risorsa
...
Se il tuo programma usa un database, il fornitore del database ti darà le librerie che potrai usare
...
dall'archivio ed inclusi nell'eseguibile finale. Poiché il linker cerca l'archivio solo quando lo
...
indefiniti. Piuttosto, i file oggetto che compongono la libreria condivisa sono combinati in un
...
in diversi programmi, così il codice nell'oggetto condiviso non deve
...
Fare il link con una libreria condivisa è come fare il link con un archivio statico. Per
esempio, la seguente riga farà il link con libtest.so se esso è nella directory corrente
...
che queste librerie siano disponibili quando l'eseguibile è in esecuzione. Nota che \texttt{ldd} elencherà
...
generalmente mostra la scarsità di design, ma lo fa occasionalmente.
In questo caso, nella riga di comando puoi indicare più volte una singola libreria.
Il linker cercherà
...
Il principale vantaggio di una libreria condivisa è quello di risparmiare spazio sul sistema nel
quale il programma è installato. Se stai installando 10 programmi, ed essi fanno tutti uso della
...
Ciò evita che il compilatore C++ ``distrugga'' il nome della funzione,
